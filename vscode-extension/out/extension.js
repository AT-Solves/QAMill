...
let backendProcess;
let dashboardPanel;
let studioPanel; // QAMill Test Studio
let statusBarItem;
...
let lastProjectRoot;
let pendingJob;
let activeStreamReq; // Node.js HTTP stream reader
...
function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand("amil.runAnalysis", (uri) => runAnalysis(context, uri)), vscode.commands.registerCommand("amil.stopAnalysis", stopAnalysis), vscode.commands.registerCommand("amil.selectLLM", selectLLM), vscode.commands.registerCommand("amil.setIdentity", setIdentity), vscode.commands.registerCommand("amil.generateUnitTests", (uri) => openTestStudio(context, uri, "unit")), vscode.commands.registerCommand("amil.generateManualTests", (uri) => openTestStudio(context, uri, "test_case")), vscode.commands.registerCommand("amil.openTestStudio", (uri) => openTestStudio(context, uri)));
}
function deactivate() {
    stopAnalysis();
}
...
async function runAnalysis(context, uri) {
    ...
    if (/^test_|_test\.(py|js|ts|jsx|tsx)$|\.test\.(js|ts|jsx|tsx)$|\.spec\.(js|ts|jsx|tsx)$/i.test(basename)) {
        const pick = await vscode.window.showWarningMessage(`QAMill: "${basename}" looks like a test file. Select the SOURCE file for meaningful mutation testing.`, "Analyze Anyway", "Cancel");
...
function openDashboard(context, port) {
    ...
    dashboardPanel = vscode.window.createWebviewPanel("amilDashboard", "QAMill QA Governance — Test Quality", vscode.ViewColumn.Beside, {
        enableScripts: true,
    ...
    dashboardPanel.webview.onDidReceiveMessage(async (msg) => {
        ...
        if (msg.type?.startsWith("sh_")) {
            ...
            const backendPort = cfg.get("backendPort", 8765);
        ...
        if (msg.type === "run_analysis") {
            ...
            // Fall back to active editor — but skip test files
            const activeFile = vscode.window.activeTextEditor?.document.fileName;
            const isSourceFile = activeFile
                ? /\.(py|js|ts|jsx|tsx)$/i.test(activeFile)
                : false;
            const isTestFile = activeFile
                ? /^test_|_test\.(py|js|ts|jsx|tsx)$|\.test\.(js|ts|jsx|tsx)$|\.spec\.(js|ts|jsx|tsx)$|conftest\.py$/i.test(activeFile)
                : false;
            const targetFile = lastFilePath
                || (isSourceFile && !isTestFile ? activeFile : undefined);
            const targetRoot = lastProjectRoot
                || vscode.workspace.workspaceFolders?.[0]?.uri.fsPath;
            if (!targetFile || !targetRoot) {
                const msg2 = "Open a source file (Python, JavaScript, TypeScript) in the editor, then click Run Analysis.";
...
async function setIdentity() {
    ...
    if (choice.value === "__signin__" || choice.value === "__signup__") {
        ...
        try {
            ...
            const resp = await postJson(`http://localhost:${port}${endpoint}`, isSignup ? { email, password, name } : { email, password });
            const user = resp.user;
            const cfg = vscode.workspace.getConfiguration("amil");
            const t = vscode.ConfigurationTarget.Workspace;
            await cfg.update("userEmail", user.email, t);
            ...
            vscode.window.showInformationMessage(`QAMill: ${isSignup ? "Account created" : "Signed in"} as ${user.email}`);
        }
        catch (err) {
            const m = String(err).replace(/^Error: HTTP \d+: /, "");
            vscode.window.showErrorMessage(`QAMill: ${m}`);
...
let studioTarget;
function openTestStudio(context, uri, presetFormat) {
    // Resolve the target .py file
    let filePath = uri?.fsPath;
    if (!filePath) {
        const ed = vscode.window.activeTextEditor;
        const langId = ed?.document.languageId;
        if (ed && ["python", "javascript", "typescript", "javascriptreact", "typescriptreact"].includes(langId || "")) {
            filePath = ed.document.uri.fsPath;
        }
    }
    if (!filePath || !/\.(py|js|ts|jsx|tsx)$/i.test(filePath)) {
        vscode.window.showErrorMessage("QAMill: Open or right-click a source file (Python, JavaScript, TypeScript) to open Test Studio.");
        return;
    }
    ...
    const logoUri = vscode.Uri.joinPath(context.extensionUri, "media", "qamill-logo.png");
    studioPanel = vscode.window.createWebviewPanel("amilTestStudio", "QAMill QA Governance — Test Authoring", vscode.ViewColumn.Active, { enableScripts: true, retainContextWhenHidden: true,
    ...
    studioPanel.webview.onDidReceiveMessage(async (msg) => {
        ...
        if (msg.type === "studio_generate") {
            await runStudioGeneration(context, msg);
            return;
        }
        if (msg.type === "studio_save") {
            await saveStudioResult(msg);
            return;
        }
    });
...
function buildStudioInit() {
    ...
    return {
        ...
        provider: cfg.get("llmProvider", "inhouse"),
        preset: studioTarget?.presetFormat || "test_case",
    };
}
async function runStudioGeneration(context, msg) {
    const cfg = vscode.workspace.getConfiguration("amil");
    const port = cfg.get("backendPort", 8765);
    ...
    const prog = (m) => studioPanel?.webview.postMessage({ type: "studio_progress", message: m });
    prog("Starting analysis engine…");
    const ready = await ensureBackendRunning(context, port);
    if (!ready) {
        studioPanel?.webview.postMessage({ type: "studio_error", message: "Could not start the backend." });
        return;
    }
    ...
    if (fmt === "unit") {
        prog("Will verify the suite against your original code after generation…");
    }
    try {
...
async function saveStudioResult(msg) {
    const filePath = studioTarget?.filePath;
    if (!filePath || !msg.content) {
        return;
    }
...
async function refreshIdentity(port) {
    ...
    if (isNewSignIn && !signInNotified) {
        ...
        vscode.window.showInformationMessage(`QAMill: Signed in as ${email}` +
 ...
 */
async function startExtensionStream(rawUrl) {
...
async function handleSharedHeaderMessage(panel, msg, context, port) {
    ...
    if (msg.type === "sh_connect_provider") {
        ...
        try {
            ...
            if (result.valid) {
                ...
                const connectPayload = { provider, api_key };
                if (model) {
                    connectPayload.model = model;
                }
    ...
    if (msg.type === "sh_disconnect_provider") {
        ...
        try {
            const ready = await ensureBackendRunning(context, port);
            if (ready) {
                await postJson(`http://127.0.0.1:${port}/auth/llm/disconnect`, { provider });
            }
    ...
    if (msg.type === "sh_set_active_provider") {
        ...
        try {
            const ready = await ensureBackendRunning(context, port);
            if (ready) {
                await postJson(`http://127.0.0.1:${port}/auth/llm/set-active`, { provider });
            }
            panel?.webview.postMessage({ type: "sh_refresh_providers" });
        }
        catch (e) {
...
function getSharedHeaderHtml(tabType, logoUri) {
    const subtitles = {
...
// ── Test Studio HTML ───────────────────────────────────────────────────────────
function getTestStudioHtml(cspSource, logoUri) {
...
function getDashboardHtml(port, logoUri = "", cspSource = "") {
